...
describe('MahjongEngine', () => {
  ...
  describe('getPlayerBase', () => {
    const createMockState = (dealerIndex: number, streaks: number[]): GameState => ({
      players: playerNames.map((name, i) => ({ name, streak: streaks[i], history: [] })),
      baseScore,
      dealerIndex,
  ...
  describe('calculateRoundChanges', () => {
    ...
    it('should apply multiplier (x2) to payouts', () => {
      const state = MahjongEngine.createInitialState(playerNames, baseScore, 0);
      const changes = MahjongEngine.calculateRoundChanges(state, 1, 2, [0, 0, 0, 0]);
    ...
    it('should handle Water settlement (Field Points)', () => {
      ...
      const changes = MahjongEngine.calculateRoundChanges(state, 1, 1, fieldPoints);
